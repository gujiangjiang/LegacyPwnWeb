...
* 🌐 **多语言与国际化 (i18n):** 提供基于独立 JSON 语言包的中英双语无缝切换。通过 XHR 动态拉取配置与本地存储记忆，实现彻底的数据解耦。
* 🔍 **环境智能检测与拦截:** 通过解析 User-Agent 自动识别设备族类与 iOS 版本，实时提示兼容性。在 UI 层禁用按钮的基础上，底层 `wkloader` 核心更加入了严格的 OS 主版本过滤，彻底阻断在非 iOS 8/9 设备上引发的不合法载荷请求与报错，并优化了加载顺序，大幅节省了无效请求带来的服务器带宽压力。
...
```text
LegacyPwnWeb/
├── index.html                   # 现代化深色主题仪表盘 (UI 前端，内置交互弹窗与 i18n 标签)
└── assets/
    ├── css/                     # 样式文件目录
    │   └── style.css            # 全局及弹窗组件样式表
    ├── certs/                   # 证书文件目录
    │   └── beeg.mobileconfig    # 越狱必备的信任证书文件
    ├── lang/                    # 独立的国际化语言包目录 (JSON 格式)
    │   ├── zh.json
    │   └── en.json
    ├── js/
    │   ├── main.js              # 页面 UI 交互控制、设备检测与动态 JSON 字典加载引擎
